...
def _parse_errors(text: str, target: str) -> list[SkriptError]:
    lines = [clean_line(line) for line in text.splitlines()]
    errors: list[SkriptError] = []

    for index, line in enumerate(lines):
        match = _LINE_RE.search(line)
        if not match or not _same_script(match.group(2), target):
            continue
        ...
        for following in lines[index + 1 :]:
            ...
            message = following
            break
        errors.append({"line": int(match.group(1)), "message": message})

    return errors


def parse_reload_output(rcon_output: str, log_output: str, target: str) -> CheckResult:
    ...
    for error in _parse_errors(combined, target):
    ...
    success_terminal = any(
        ...
        for match in _SUCCESS_RE.finditer(clean_line(combined))
    )
    error_count = sum(
        int(match.group(1))
        for match in _ERROR_SUMMARY_RE.finditer(clean_line(combined))
    ...
    if error_count and not errors:
        ...
        errors.append(
            {"line": 0, "message": "Skript could not reload the requested script."}
        )
